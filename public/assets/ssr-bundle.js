...
`;
  ...
  function burnCpu(ms) {
    const iterations = Math.max(0, Math.floor(ms)) * 2e5;
    let sink = 0;
    for (let i = 0; i < iterations; i += 1) {
      sink = sink + (i & 7) & 65535;
    }
    globalThis.__HYDRA_BURN_SINK__ = sink;
  }
  function nextIsolateCounter() {
    const key = "__HYDRA_INTERNAL_ISOLATE_COUNTER__";
    const current = asFiniteNumber(globalThis[key]) ?? 0;
  ...
  globalThis.render = (url, propsJson, requestContextJson) => {
    ...
    const appHtml = redirect ? "" : renderToString(/* @__PURE__ */ jsxRuntimeExports.jsx(App, { url: route.routeUrl, initialProps: hydratedProps }));
